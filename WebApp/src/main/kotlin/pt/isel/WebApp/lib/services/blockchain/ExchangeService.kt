...
class ExchangeService(blockchain_url : String, contract_address: String? = null) : IExchangeHolder {
    ...
    override suspend fun newExchange(orderId: String, price: Long, destinationAddress: String, end_date: String) =
        coroutineScope {
            return@coroutineScope exchangeHolder.newExchange(
                BigInteger(orderId),
                BigInteger(price.toString()),
                destinationAddress,
                BigInteger(end_date)
            ).sendAsync()
        }
